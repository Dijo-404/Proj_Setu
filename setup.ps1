...
$StartScript = Join-Path $ProjectRoot "start_setu.ps1"
...
function Offer-ServiceInstall {
    ...
    if (-not (Test-AdminShell)) {
        Write-Host "Skipping service install because this PowerShell window is not running as Administrator." -ForegroundColor Yellow
        Write-Host "Run setup again as Administrator if you want the service installed."
        return
    }
...
if (-not $SkipStart) {
    $startNow = Read-YesNo "Start Setu now in a new PowerShell window?" $true
    if ($startNow) {
